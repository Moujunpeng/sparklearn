...
class SparkTest extends AnyFunSuite {
  ...
  test("partition test") {
    ...
    // id + 100 map(iter=> (iter._1,iter._2))
    val l = df.rdd.partitions.size
    println("rdd partition is " + l)
  ...
  test("testParalize"){
    ...
    // 创建rdd
    val rdd = session.sparkContext.parallelize(buffer)

    println("rdd partition size is " + rdd.partitions.size)
